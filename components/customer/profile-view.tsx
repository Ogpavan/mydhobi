...
export function ProfileView({ user }: { user: AuthUser }) {
  ...
  async function saveProfile(event:React.FormEvent<HTMLFormElement>){
    ...
    setProfile(data.user);setEditOpen(false);toast.success("Profile updated");router.refresh();
  }

  async function deleteAccount(){
    if(deleteText!=="DELETE"){toast.error("Type DELETE to confirm");return;}
    setDeleting(true);const response=await fetch("/api/customer/profile",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmation:deleteText})});const data=await response.json() as {message?:string};if(!response.ok){toast.error(data.message??"Unable to delete account");setDeleting(false);return;}startNavigationProgress();router.replace("/");
  }
