import { NextResponse } from "next/server";

import { JWT_COOKIE_NAME, JWT_MAX_AGE_SECONDS, signAuthToken } from "@/lib/auth";
import { pool } from "@/lib/db";
import { getCurrentUser } from "@/lib/session";

export async function PATCH(request: Request) {
  const user = await getCurrentUser();
  if (!user || user.role !== "customer") return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  const body = await request.json() as Record<string, unknown>;
  const name = typeof body.name === "string" ? body.name.trim() : "";
  const mobile = typeof body.mobile === "string" ? body.mobile.trim() : "";
  if (name.length < 2 || name.length > 100) return NextResponse.json({ message: "Enter your name." }, { status: 400 });
  if (!/^\d{10}$/.test(mobile)) return NextResponse.json({ message: "Enter a 10-digit mobile number." }, { status: 400 });
  try {
    await pool.query("UPDATE app_users SET name=$2,mobile=$3 WHERE id=$1", [user.id,name,mobile]);
    const updated={...user,name,mobile};
    const response=NextResponse.json({user:updated});
    response.cookies.set({name:JWT_COOKIE_NAME,value:await signAuthToken(updated),httpOnly:true,sameSite:"lax",secure:process.env.NODE_ENV==="production",path:"/",maxAge:JWT_MAX_AGE_SECONDS});
    return response;
  } catch (error) {
    if (typeof error === "object" && error && "code" in error && error.code === "23505") return NextResponse.json({ message: "This mobile number is already in use." }, { status: 409 });
    return NextResponse.json({ message: "Unable to update profile." }, { status: 500 });
  }
}

export async function DELETE(request: Request) {
  const user=await getCurrentUser();
  if(!user||user.role!=="customer")return NextResponse.json({message:"Unauthorized"},{status:401});
  const body=await request.json() as Record<string,unknown>;
  if(body.confirmation!=="DELETE")return NextResponse.json({message:"Type DELETE to confirm."},{status:400});
  const client=await pool.connect();
  try{
    await client.query("BEGIN");
    await client.query("DELETE FROM customers WHERE user_id=$1",[user.id]);
    await client.query("DELETE FROM app_users WHERE id=$1",[user.id]);
    await client.query("COMMIT");
    const response=NextResponse.json({success:true});
    response.cookies.delete(JWT_COOKIE_NAME);
    return response;
  }catch(error){await client.query("ROLLBACK");console.error("Delete customer account failed",error);return NextResponse.json({message:"Unable to delete account."},{status:500});}finally{client.release();}
}
