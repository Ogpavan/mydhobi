...
import { createServiceCategory, listServiceCategories } from "@/lib/service-catalog";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ category: await createServiceCategory({ name, imagePath: imagePath.slice(0, 250), displayOrder }) }, { status: 201 });
  } catch (error) {
