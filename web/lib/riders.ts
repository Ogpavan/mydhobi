    ...
    SELECT riders.id,riders.name,riders.mobile,riders.area,riders.status,
      (
        SELECT COUNT(*) FROM (
          ...
          FROM order_pickups pickups WHERE pickups.rider_id=riders.id
          UNION ALL
          SELECT deliveries.id,deliveries.scheduled_at
          FROM order_deliveries deliveries WHERE deliveries.rider_id=riders.id
        ) jobs
...
export async function getRider(id: string): Promise<RiderDetails | null> {
  await ensureDeliverySchema();
  const { rows } = await pool.query(
    `SELECT riders.id,riders.name,riders.mobile,riders.area,riders.status,
      0::int AS today_jobs,0::int AS active_jobs
     FROM riders WHERE riders.id=$1 LIMIT 1`,
    [id],
  );
...
export async function createRider(input: {
  ...
  const { rows } = await pool.query(
    ...
    [input.name, input.mobile, input.area, input.status],
  );
